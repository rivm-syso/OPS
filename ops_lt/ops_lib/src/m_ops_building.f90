...
private
public interpol_2d, split1, split2
...
character(len=200), parameter :: buildingParamNames(9) = &
        (/'hEmis              ', &
          'V_stack            ', &
          'D_stack            ', &
          'buildingHeight     ', &
...
Type Tbuilding
   real      :: length = 0.0                              ! building length [m]
   real      :: width  = 0.0                              ! building width [m]
   real      :: height = 0.0                              ! building height [m]
   real      :: orientation = 0.0                         ! building orientation (degrees w.r.t. x-axis and long side, positive for counter clockwise)
   real      :: x      = 0.0                              ! building center x coordinate
   real      :: y      = 0.0                              ! building center y coordinate
   ...
   integer   :: type = 0                                  ! building type for determining distance function for building effect [-]; type = 0 -> no building effect
End Type Tbuilding 
...
call ops_building_read_building_factors(nClassProd, buildingEffect%buildingFactArray, error)
if (error%haserror) goto 9999
...
!-----------------------------------------------------------------------------------------
subroutine ops_building_read_building_factors(nClassProd, buildingFactArray, error)
