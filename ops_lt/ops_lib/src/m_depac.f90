...
! if a new species is introduced, all these CASE constructs have to be checked.
! select case(comp_id)
! case(i_HNO3)
! case(i_NO)
! case(i_NO2)
...
!    print *, 'component ',comp_id,' not supported'
!    stop 1
! end select
!
! For convenience, case entries may be combined, e.g. case(i_NO, i_NO2).
! In fact, this type of CASE construct is the only allowed construct where comp_id may occur.
!
!****************************************************************************************
...
! that should be kept private. Only 'use' m_depac, never this module
! directly, except in m_depac itself and in its unit tests.
...
!-------------------------------------------------------------------
subroutine rc_getpar(comp_id,diffc,rsoil_wet,rsoil_frozen,ipar_snow,rsoil)
...
!-------------------------------------------------------------------
subroutine rc_special(comp_id,lu,t,ipar_snow,nwet,rc_tot,ready,ccomp_tot)
...
!-------------------------------------------------------------------
subroutine rc_gw(comp_id,iratns,t,rh,nwet,sai,gw, SAI_present)
...
case(i_NO2)
    call rw_constant(2000.,gw, SAI_present)

case(i_NO)
   call rw_constant(-999.,gw, SAI_present)  ! see Erisman et al, 1994 section 3.2.3
       
case(i_O3)
   call rw_constant(1000.,gw, SAI_present)

case(i_SO2)
   call rw_so2(t,nwet,rh,iratns,gw, SAI_present)

case(i_NH3)
   call rw_nh3_sutton(t,rh,gw, SAI_present)
...
!-------------------------------------------------------------------
subroutine rw_so2(t,nwet,rh,iratns,gw, SAI_present)
...
!-------------------------------------------------------------------
subroutine rw_nh3_sutton(tsurf,rh,gw, SAI_present)
...
!-------------------------------------------------------------------
subroutine rw_constant(rw_val,gw, SAI_present)
...
!-------------------------------------------------------------------
subroutine rc_gstom(comp_id,lu,lai,glrad,sinphi, t,rh,diffc,gstom, LAI_present)
...
logical, intent(in)  :: LAI_present !  vegetation is present 
! variables from module
...
!-------------------------------------------------------------------
SUBROUTINE rc_gstom_emb(lu,glrad,T,vpd,lai,sinp,Gsto, LAI_present)
...
!     Calculate fraction of PAR in the direct beam
      ...
      fv=max(0.01, rdu/rv*(1.0-(fv**2)**(1.0/3.0))) ! Should be optimized better by ifort
...
! Addo van Pul and Adrie Jacobs, 1993, BLM
!-------------------------------------------------------------------
...
!-------------------------------------------------------------------
subroutine rc_comp_point(comp_id,lu,day_of_year,t,catm,c_ave_prev_nh3, &
       c_ave_prev_so2,gamma_soil_water_fac,gw,gstom,gsoil_eff, &
       gc_tot,ccomp_tot, LAI_present, SAI_present)
...
logical, intent(in) :: LAI_present, SAI_present
! Variables from module:
...
!**********************************************************************************
!
! main routines depac_prepare and depac_finish
!
! compute total canopy (or surface) resistance Rc for gases
! compute effective Rc based on compensation points, incl. new parameterisation Rw
!
!**********************************************************************************

!------------------------------------------------------------------------------------
! depac_prepare: prepare to compute total canopy (or surface) resistance Rc for gases
! This subroutine only has to be called once for a tra/rcp combination
!------------------------------------------------------------------------------------
subroutine depac_prepare(comp_id, day_of_year, lat, t, glrad, sinphi, rh, nwet, lu, iratns, &
...
integer         , intent(in)  :: iratns       ! index for NH3/SO2 ratio;
                                              ...
                                              ! iratns = 3: very low NH3/SO2
real            , intent(out) :: lai          ! one-sided leaf area index (-)
real            , intent(out) :: sai          ! surface area index (-) (lai + branches and stems)
logical         , intent(out) :: ready        ! Rc has been set
real            , intent(out) :: rsoil_wet    ! soil resistance for wet soil
real            , intent(out) :: rsoil_frozen ! soil resistance for frozen soil
real, dimension(nlu), intent(out) :: rsoil    ! soil resistance
...
if (.not. ready) then
   ...
   call rc_gstom(comp_id,lu,lai,glrad,sinphi,t,rh,diffc,gstom, LAI_present) 

endif

end subroutine depac_prepare


!---------------------------------------------------------------------------------
! depac_finish: finish computing total canopy (or surface) resistance Rc for gases
! This subroutine has to be called twice, for tra/rcp separately, only if
! depac_prepare ready output is .false.
!---------------------------------------------------------------------------------
subroutine depac_finish(comp_id, day_of_year, t, ust, nwet, lu, lai, sai, rsoil_wet, rsoil_frozen, &
                        rsoil, gw, gstom, c_ave_prev_nh3, c_ave_prev_so2, catm, gamma_soil_water_fac, &
                        ra, rb, rc_tot, ccomp_tot, rc_eff)
...
real            , intent(in)     :: rsoil_wet      ! soil resistance for wet soil
real            , intent(in)     :: rsoil_frozen   ! soil resistance for frozen soil
real, dimension(nlu), intent(in) :: rsoil          ! soil resistance
real            , intent(in)     :: gw             ! external leaf conductance (m/s) 
real            , intent(in)     :: gstom          ! stomatal conductance (m/s)
...
real            , intent(inout)  :: rc_tot         ! total canopy resistance Rc (s/m)
real, optional  , intent(inout)  :: ccomp_tot      ! total compensation point (ug/m3)
...
! Compensation points:
call rc_comp_point(comp_id,lu,day_of_year,t,catm,c_ave_prev_nh3,c_ave_prev_so2, &
        gamma_soil_water_fac,gw,gstom,gsoil_eff,gc_tot,ccomp_tot, LAI_present, SAI_present)

! Effective Rc based on compensation points:
call rc_comp_point_rc_eff(ccomp_tot,catm,ra,rb,rc_tot,rc_eff)

end subroutine depac_finish

! ----------------------------------------------------------------------------------
! DEPAC
! Call depac_prepare and depac_finish; kept for testing.
! Previous version had optional arguments; in OPS all arguments are always provided.
! ----------------------------------------------------------------------------------
subroutine depac(comp_id, day_of_year, lat, t, ust, glrad, sinphi, rh, nwet, lu, iratns, rc_tot, &
...
integer         , intent(in)  :: iratns
real            , intent(out) :: rc_tot
real            , intent(in)  :: c_ave_prev_nh3
real            , intent(in)  :: c_ave_prev_so2
real            , intent(in)  :: catm
real            , intent(in)  :: gamma_soil_water_fac
real            , intent(out) :: ccomp_tot
real            , intent(in)  :: ra
real            , intent(in)  :: rb
real            , intent(out) :: rc_eff
real                          :: lai
real                          :: sai
logical                       :: ready
real                          :: rsoil_wet
real                          :: rsoil_frozen
real, dimension(nlu)          :: rsoil
real                          :: gw
real                          :: gstom

call depac_prepare(comp_id, day_of_year, lat, t, glrad, sinphi, rh, nwet, lu, iratns, &
                   lai, sai, ready, rsoil_wet, rsoil_frozen, rsoil, gw, gstom, rc_tot, ccomp_tot)
if (.not. ready) then
   call depac_finish(comp_id, day_of_year, t, ust, nwet, lu, lai, sai, rsoil_wet, rsoil_frozen, &
                     rsoil, gw, gstom, c_ave_prev_nh3, c_ave_prev_so2, catm, gamma_soil_water_fac, &
                     ra, rb, rc_tot, ccomp_tot, rc_eff)
endif
