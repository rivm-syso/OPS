...
!------------------------------------------------------------
subroutine ops_plumerise_prelim(istab, isec_prelim, astat, hemis0, qw, D_stack, V_stack, Ts_stack, emis_horizontal, varin_meteo, varin_unc, hemis1, error)
...
type(Tvarin_unc), intent(in)   :: varin_unc   ! uncertainty variables
...
call ops_plumerise(-999., hemis0, -999., -999., qw, D_stack, V_stack, Ts_stack, emis_horizontal, temp_C, -999., -999., &  
                   varin_meteo, varin_unc, hemis1, dum, error, prelim, istab, isec_prelim, astat)
...
!------------------------------------------------------------
subroutine ops_plumerise( &
   z0, hemis0, uster, ol, qw, D_stack, V_stack, Ts_stack, emis_horizontal, temp_C, zmix, &
   zmix_loc, varin_meteo, varin_unc, hemis1, onder, error, prelim, istab, isec_prelim, astat)
...
type(Tvarin_meteo), intent(in) :: varin_meteo ! meteo_variables
type(Tvarin_unc), intent(in)   :: varin_unc   ! Parameter noise for uncertainty analysis.
...
if (present(prelim)) then
   if (.not. prelim) then
       call SetError('You specify prelim=.true. or leave prelim out, but you cannot set prelim=.false.', error)
       goto 9999
   end if
endif
...
if (present(prelim)) then
   ...
   call ops_wv_log_profile(z0,hemis0,uster,ol,varin_meteo,u_stack)
endif
...
if (present(prelim)) then
   call ops_plumerise_buoyancy(z0,ol,uster,varin_meteo,non_stable,qw2,Ta_stack,dthetadz_stable,u_stack,hemis0,dh_buoyancy,prelim,istab,isec_prelim,astat) 
else
   call ops_plumerise_buoyancy(z0,ol,uster,varin_meteo,non_stable,qw2,Ta_stack,dthetadz_stable,u_stack,hemis0,dh_buoyancy) 
endif
...
if (.not. is_missing(V_stack) .and. .not. emis_horizontal) then
    ...
    if (hemis0 .lt. z_u_threshold) then
       if (present(prelim)) then
          ...
          call ops_wv_log_profile(z0,z_u_threshold,uster,ol,varin_meteo,u_threshold)
       endif
...
! Multiply the plumerise by a noise factor. When the noise follows a log-normal
! distribution, this is equivalent to adding normal noise in log-space (`exp(log(dh) + e)`).
! Using this approach `dh` is guaranteed to remain positive without truncating values.
dh = dh * varin_unc%plrise_fact

hemis1 = hemis0 + dh
! write(*,'(a,4(1x,e12.5))') 'in routine ops_plumerise c: ',hemis0,hemis1,hemis1-hemis0,-999.0

! 6. plume penetration
if (.not. present(prelim)) call ops_plume_penetration(hemis0,zmix,zmix_loc,ol,dh,hemis1,onder) 
   ...
   if (is_missing(Ts_stack)) then
      ...
      if (emis_horizontal) then
         ...
         if (qw .eq. 0.0) then
            ...
            ! Check:
            if (qw .ge. C1 .or. Ts_stack2-Ta_stack .lt. 0.0 .or. Ts_stack2-Ta_stack .gt. 2000.0) then  ! See also check in ops_emis_read_annual1 - check_stack_param
...
!------------------------------------------------------------
subroutine ops_plumerise_buoyancy(z0, ol, uster, varin_meteo, non_stable, qw, Ta_stack, dthetadz_stable, u_stack, hemis0, dh_buoyancy, prelim, istab, isec_prelim, astat)
!-------------------------------------------------------------------------------------------------------------------------------
! 
   ...
   do while (.not. converged .and. it .le. maxit) 
      ...
      if (.not. converged .and. it .lt. maxit) then
         ...
         if (prelim1) then
            ...
            call ops_wv_log_profile(z0,hemis0+dh_buoyancy/2,uster,ol,varin_meteo,u_plume)
         endif
...
if (V_stack .le. 0.0) then
   ...
   tmp =  (V_stack**2.)*(D_stack**2.) / (Ts_stack*u_stack)
   if (tmp < 0) then
      tmp = - ((-tmp) ** (1./3.))
   else
      tmp = tmp ** (1./3.)
   endif
   dh_stable = 0.646 * tmp * (Ta_stack**0.5) * (dthetadz_stable**(-1./6.))
