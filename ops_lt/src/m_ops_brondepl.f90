...
REAL*4,    INTENT(IN)                            :: vd_eff_trj_zra             ! 
REAL*4,    INTENT(IN)                            :: xl                         ! 
...
REAL*4,    INTENT(IN)                            :: qbstf                      ! source strength current source (for current particle class) [g/s]
REAL*4,    INTENT(IN)                            :: vd_trj_z0                  ! deposition velocity for trajectory, height z0 [m/s]; used in source depletion factor cq2
REAL*4,    INTENT(IN)                            :: onder                      ! fraction of mass inside the mixing layer. Mass flux of that part of the plume that is below the mixing layer 
...
REAL*4,    INTENT(INOUT)                         :: cgt                        ! gradient factor at 4 m height [-]
REAL*4,    INTENT(INOUT)                         :: cgt_z                      ! gradient factor at receptor height zm [-]
...
REAL*4,    INTENT(OUT)                           :: a                          ! 
REAL*4,    INTENT(OUT)                           :: cq1                        ! source depletion ratio for all removal processes for phase 1 (area source)
...
REAL*4                                           :: uxg                        ! 
REAL*4                                           :: s2                         ! 
REAL*4                                           :: k_drydep_src               ! dry deposition rate = vd/sigma_z for area source, 4 m height [1/s]
REAL*4                                           :: sh                         ! 
REAL*4                                           :: al                         ! 
...
ELSE
   ...
   !             x
   !             /  vd(z)                  (x - xg) vd(z)
   ! cdn = exp[- | ------- dksi ] = exp[-  -------------- ], in which we substitute x = disx, vd(z) = vd(zra) = vd(50) = vd_eff_trj_zra, u = ueff
   !             /  u xl                        u xl
   !           ksi=xg
   !
   !            
...
IF (radius .GT. (0. + EPS_DELTA)) THEN
  ...
  IF (disx .GT. (radius + EPS_DELTA)) THEN
    ...
    hf = (sigzr/4 + htot + 6.)/2. 
    CALL ops_wv_log_profile(z0_src, hf, uster_src, ol_src, uxr)
...
! C(x) : concentration at receptor, taking into account the gradient due to deposition = cxx*(1-cgt)
...
!
! source code:
!                k_drydep_src*dxeff           vd/sigma_z (S/4) exp(-a/18)           vd (S/4) exp(-((vchem+vnatpri+vd/sigma_z) S)/(18 uxr ))     
!   cq1 = EXP( - ------------------) = EXP( - ---------------------------) = EXP( - --------------------------------------------------------)   
!                      uxr                               uxr                                      uxr sigma_z                                   
!
...
!
! help variable a = k*t = (k_chem + k_wetdep + k_drydep_src)*(diameter/(2*uxr)) =  
