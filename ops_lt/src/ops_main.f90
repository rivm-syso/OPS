...
! ops_read_bg            : Read background concentrations and other chemical maps for SO2, NH3, NO2
...
CALL ops_read_ctr(project, runid, year, icm, namco, amol1, gasv, idep, kdeppar, ddeppar, knatdeppar, wdeppar, dg, irev,        &
               ...
               &  z0_user, intpol, ideh, igrid, checked, f_z0user, isec, nsubsec, chem_meteo_prognosis, error)
IF (error%haserror) GOTO 1000 ! GOTO error handling at end of program
!
...
IF (isec) THEN
  ...
  CALL ops_read_bg(icm, iopt_vchem, nsubsec, year, chem_meteo_prognosis, so2bggrid, no2bggrid, nh3bggrid, f_subsec_grid, &
